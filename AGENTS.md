...
- **Workflow (Anti-Piggybacking):**
  ...
  7. **COMPLETE CLEANUP (LOCAL & REMOTE):** Immediately after the PR is merged, the Agent MUST execute `git checkout dev` and `git pull`. Then, the Agent MUST forcefully delete the temporary local branch (`git branch -D <branch-name>`) AND delete the remote branch (`git push origin --delete <branch-name>` or via `gh pr merge -d`). The Agent must NEVER leave trash branches on the remote repository and must NEVER start a new task from a temporary branch.
  8. **ABSOLUTE RULE (NO EXCEPTIONS):** No matter how small the change is (even a single character typo or a 1-line configuration fix), you MUST NEVER commit directly to the `dev` branch. Every single change MUST be isolated in its own branch and submitted via a Pull Request.
