...
- **Workflow (Anti-Piggybacking):**
  ...
  7. **LOCAL CLEANUP (SYNC DEV):** Immediately after the PR is merged, the Agent MUST execute `git checkout dev`, `git pull`, and forcefully delete the temporary local branch (`git branch -D <branch-name>`). The Agent must NEVER start a new task from a temporary branch.
