...
- **Workflow (Anti-Piggybacking):**
  ...
  6. **ABSOLUTE RULE (NO EXCEPTIONS):** No matter how small the change is (even a single character typo or a 1-line configuration fix), you MUST NEVER commit directly to the branch. Every single change MUST be isolated in its own branch and submitted via a Pull Request.
